...
@synthesize arrCategory,intSelectedCatIndex;
...
#pragma mark - Bind Category
-(void)bindCategory
{
    ////Remove all subviews first////
    NSArray* subviews = [[NSArray alloc] initWithArray: _scrCategory.subviews];
    ...
    for (int i = 0; i < [self.arrCategory count]; i++) {
        NSDictionary *dicCategory = [self.arrCategory objectAtIndex:i];
        
        CGRect frame;
        frame.origin.x = fltX;
        frame.origin.y = 0.0;
        frame.size.width = 100.0;
        frame.size.height = 35.0;   //33.0
        
        UIButton *btnCategory=[UIButton buttonWithType:UIButtonTypeCustom];
        [btnCategory setFrame:frame];
        [btnCategory setTitle:[(NSString *)([dicCategory objectForKey:@"name"]) uppercaseString] forState:UIControlStateNormal];
        [btnCategory.titleLabel setFont:[UIFont fontWithName:@"Arial" size:13.0]];
        //[btnCategory setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
        [btnCategory setTag:i];
        [btnCategory addTarget:self action:@selector(btnTappedCategory:) forControlEvents:UIControlEventTouchUpInside]; //UIControlEventTouchDown
        
        if(i==self.intSelectedCatIndex){
            [btnCategory setBackgroundColor:[UIColor blackColor]];
            //[btnCategory setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
            [btnCategory setTitleColor:[UIColor colorWithRed:255.0/255.0 green:205.0/255.0 blue:56.0/255.0 alpha:1.0] forState:UIControlStateNormal];
        }
        else{
            //[btnCategory setBackgroundColor:[UIColor yellowColor]];
            [btnCategory setBackgroundColor:[UIColor colorWithRed:255.0/255.0 green:205.0/255.0 blue:56.0/255.0 alpha:1.0]];
            [btnCategory setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        }
        
        UILabel *lblSeparator = [[UILabel alloc] init];
        [lblSeparator setFrame:CGRectMake(fltX-1, frame.origin.y, 1, frame.size.height)];
        lblSeparator.backgroundColor = [UIColor blackColor];
        lblSeparator.alpha = 0.6;
        lblSeparator.text = @"";
        [_scrCategory addSubview:lblSeparator];
        
        
        [_scrCategory addSubview:btnCategory];
        //[btnCategory release];
        
        fltX = fltX+100.0;
    }
    _scrCategory.contentSize = CGSizeMake(fltX, _scrCategory.frame.size.height);
}
#pragma mark Button Tapped Category
-(IBAction)btnTappedCategory:(id)sender
{
    self.intSelectedCatIndex = ((UIButton *)sender).tag;
    
    //BettingSites *objBettingBonusTmp = [self.arrCategory objectAtIndex:((UIButton *)sender).tag];
    //[FunctionManager openUrlInSafari:objBettingBonusTmp.strWebUrl];
...
-(void)fetchProducts{
    ...
    NSDictionary *dicCategory = [self.arrCategory objectAtIndex:self.intSelectedCatIndex];
    int intCatId = [[dicCategory objectForKey:@"category_id"] intValue];
    ...
    parameters[@"method"] = @"getProductList";
    
    parameters[@"userid"] = [NSString stringWithFormat:@"%d", appDelegate.objUser.intUserID];
    parameters[@"category_id"] = [NSString stringWithFormat:@"%d", intCatId];
